...
local function try_add_device(driver, device_dni, device_ip)
  ...
  if not credential then
    if driver.datastore.discovery_cache[device_dni] and driver.datastore.discovery_cache[device_dni].credential then
      log.info(string.format("load stored credential. This may have expired. dni= %s, ip= %s", device_dni, device_ip))
      credential = driver.datastore.discovery_cache[device_dni].credential
    else
